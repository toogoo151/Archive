...
class BaingaIltChildController extends Controller
{
    ...
    public function NewChildBaingIlt(Request $req)
    {
        ...
        //  хэрэглэгчийн ID folder
        $userFolder = "public/doc/BaingaIlt/{$userId}";
        $fullURL = "";

        //  0. Folder байхгүй бол үүсгэнэ
        if (!Storage::exists($userFolder)) {
            Storage::makeDirectory($userFolder);
        }

        //  1. Давхардсан файл байна уу гэдгийг эхлээд шалгана
        foreach ($req->data_url as $value) {
            ...
            if (Storage::exists($path)) {
                return response([
                    "status" => "error",
                    "msg" => "Файл \"{$setPDFPathID}\"аль хэдийн өөр газар ийм нэртэй file хадгалагдсан байна!"
                ], 422);
            }
        }

        //  2. Давхардал байхгүй бол хадгална
        foreach ($req->data_url as $value) {
            ...
            //  URL-г хэрэглэгчийн folder-той үүсгэнэ
            $getPDFUrl = 'storage/doc/BaingaIlt/' . Auth::id() . '/' . $setPDFPathID;
