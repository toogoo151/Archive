...
public function documentItemiinShaardlaga(Request $req){
    try {
    ...
    public function newDocItem(Request $req){
        try {

            // PDFtei holbootoi heseg
            if($req->documentZagvar != ""){
                $userFolder = 'public/documentItem/'.$req->missionID.'/'.$req->eeljID;
                ...
                $getPDFUrl = '/documentItem'.'/'.$req->missionID.'/'.$req->eeljID.'/'.$setImagePathID.".".$extension;
            }
            //PDFtei holbootoi heseg end duusna

            $insertDocItem = new DocumentItem();
    ...
    public function editDocItem(Request $req){
        try {

            // PDFtei holbootoi heseg
            if($req->documentZagvar != "" && strlen($req->documentZagvar) >50){
                //pdf delete
                $deletePdf = DocumentItem::find( $req->id);
    ...
    public function deleteDocItem(Request $req) {
        try {
            $delete = DocumentItem::find($req->id);
