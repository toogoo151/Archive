...
class OffResearchController extends Controller
{
    ...
    public function getOfficerResearchCheck()
    {
        try {
            $check = DB::table("pko_officer_research")
                ->where("pko_officer_research.pkoUserID", "=", Auth::user()->id)
                ->get();
            return count($check);
        } catch (\Throwable $th) {
            return response(
    ...
    {
        $check = OfficerResearch::where("pkoUserID", "=", Auth::user()->id)->get();
        if (count($check) == 0)
            return true;
        else
            return false;
    ...
    public function firstCheckOffResearch()
    {
        try {
            $newQuestion = OfficerResearch::where("pkoUserID", "=", Auth::user()->id)->count();
            if ($newQuestion > 0) {
                $newQuestionInto = OfficerResearch::where("pkoUserID", "=", Auth::user()->id)
                    ->first();
                return response(
                    array(
                        "userCheck" => count($newQuestionInto),
                    ),
                    200
                );
            } else {
    ...
    public function newofficerResearch(Request $req)
    {
        try {
            ...
            if (!$this->checkUserID(Auth::user()->id)) {
                return response(
                    array(
                        "msg" => "Та судалгаа бөглөсөн байна.",
                    ),
                    500
                );
            }
            // $users = Auth::user()->allUsersID;
            // $unitID = all_users::find(Auth::user()->allUsersID)->unitID;
            $firstN = all_users::find(Auth::user()->allUsersID)->firstName;
            $unitID = all_users::find(Auth::user()->allUsersID)->unitID;
            ...
            $newResearch->lastName = $lastN;
            $newResearch->gender = $gend;
            $newResearch->last_joinDate = $req->last_joinDate;
            $newResearch->rankTypeID = $req->rankTypeID;
            $newResearch->rankID = $req->rankID;
            $newResearch->positions = $req->positions;
            $newResearch->education = $req->education;
            $newResearch->angi_situation = $req->angi_situation;
            $newResearch->performance = $req->performance;
            $newResearch->english_score = $req->english_score;
            $newResearch->save();
            return response(
                array(
                    "status" => "success",
                    "msg" => "Таны бөглөсөн судалгааг хүлээн авлаа",
                ),
                200
            );
        } catch (\Throwable $th) {
