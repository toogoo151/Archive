...
class AllAdminController extends Controller
{
    ...
    public function getAllAdminsByTenServerSide(Request $req){
        ...
        if(Auth::user()->user_type === "superAdmin"){
            ...
            ->select( "pko_users.id", "pko_users.email","pko_users.phone", "pko_users.user_type", "all_users.comandlalID", "all_users.unitID", "all_users.firstName","all_users.lastName", "all_users.rd","all_users.position", $ageCalculationSql, "tb_gender.genderName","tb_comandlal.comandlalShortName as comandlal", "tb_unit.unitShortName as unit", "tb_ranks.shortRank",  "all_users.image")
            ...
            if($req->_seeAllUsers ==="1"){
                ...
                ->select( "pko_users.id", "pko_users.email","pko_users.phone", "pko_users.user_type", DB::raw('DATE(email_verified_at) as email_verified_date'), "all_users.comandlalID", "all_users.unitID", "all_users.firstName","all_users.lastName", "all_users.rd","all_users.position", $ageCalculationSql, "tb_gender.genderName","tb_comandlal.comandlalShortName as comandlal", "tb_unit.unitShortName as unit", "tb_ranks.shortRank",  "all_users.image")->paginate($perPage);
            }
            ...
            return response(
                array(
                    ...
                    "countAllUnitNotVerifiedUsers" => $this->countUsers(5,$req), // Цахим хаяг баталгаажуулаагүй ЦАХ чид
                    // "requestedInThisMissionData" => $admins,
                    "requestedInThisMissionData" => response()->json($admins),
        ...
        if(Auth::user()->user_type != "superAdmin"){
            if($req->_seeAllUsers ==="2"){
                ...
                ->join("all_users", function($query)use($req, $search){
                ...
                ->select( "pko_users.id", "pko_users.email","pko_users.phone", "pko_users.user_type", DB::raw('DATE(email_verified_at) as email_verified_date'), "all_users.comandlalID", "all_users.unitID", "all_users.firstName","all_users.lastName", "all_users.rd","all_users.position", $ageCalculationSql, "tb_gender.genderName","tb_comandlal.comandlalShortName as comandlal", "tb_unit.unitShortName as unit", "tb_ranks.shortRank", "all_users.image","all_users.rankParentID","all_users.rankTypeID","all_users.rankID", "all_users.foreignPass", "all_users.foreignFinishDate")
                ->paginate($perPage);
            ...
            if($req->_seeAllUsers ==="1"){
                ...
                ->select( "pko_users.id", "pko_users.email","pko_users.phone", "pko_users.user_type", DB::raw('DATE(email_verified_at) as email_verified_date'), "all_users.comandlalID", "all_users.unitID", "all_users.firstName","all_users.lastName", "all_users.rd","all_users.position", $ageCalculationSql, "tb_gender.genderName","tb_comandlal.comandlalShortName as comandlal", "tb_unit.unitShortName as unit", "tb_ranks.shortRank", "all_users.image","all_users.rankParentID","all_users.rankTypeID","all_users.rankID","all_users.gender","all_users.foreignPass", "all_users.foreignFinishDate")
                ->paginate($perPage);
            }
            ...
            return response(
                array(
                    ...
                    "requestedInThisMissionData" => response()->json($comandlalsUsers),
        ...
        if(Auth::user()->user_type === "batalionAdmin"){
            ...
            ->select( "pko_users.id", "pko_users.email","pko_users.phone", "pko_users.user_type", DB::raw('DATE(email_verified_at) as email_verified_date'), "all_users.comandlalID", "all_users.unitID", "all_users.firstName","all_users.lastName", "all_users.rd","all_users.position", $ageCalculationSql, "tb_gender.genderName","tb_comandlal.comandlalShortName as comandlal", "tb_unit.unitShortName as unit", "tb_ranks.shortRank")
            ->get();
            return  $admins; // шинэ бодолт хийгдээгүй байж магадгүй
        }
    ...
    public function countUsers($countKind, $req){
        ...
        if($countKind ==5){
            $allUnitNotVerifiedUsers = DB::table("pko_users")
                ->where("pko_users.email_verified_at", "=", null)
                ->where("pko_users.user_type", "=", "unitUser")
                ->join("all_users", function($query)use($req){
                    $getComandlalID = new all_users();
                    $query->on("pko_users.allUsersID", "=", "all_users.id");
                    if(Auth::user()->user_type != "superAdmin" || Auth::user()->user_type != "gsmafAdmin"){
                        if(Auth::user()->user_type === "comandlalAdmin"){
                            if($req->_changeUsersType === "2"){
                                $query->where("all_users.comandlalID", "=", $getComandlalID->getComandlalID())
                                ->where("all_users.unitID", "!=", $getComandlalID->getUnitlID());
                ...
                ->select( "pko_users.id")
                ->count();
            return  $allUnitNotVerifiedUsers;
        }
