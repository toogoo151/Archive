    ...
    public function getNoots($req){
        try {
            $getNoots = DB::table("pko_batalion_oron_too")
                ...
                ->leftJoin("pko_sport_score", function($query)use($req){
                    $query->on("pko_batalion_oron_too.pkoMainHistoryID", "=", "pko_sport_score.pkoMainHistoryID")
                    ->on('pko_sport_score.id', "=", DB::raw('(SELECT max(id) FROM `pko_sport_score` where `pkoMainHistoryID`=pko_batalion_oron_too.pkoMainHistoryID)'));
                })
                ->select("pko_batalion_oron_too.*", "all_users.lastName", "all_users.firstName","all_users.rd", "tb_ranks.shortRank",  "pko_missions.missionName", "pko_mission_eelj.eeljName", "pko_sport_score.sportScore as childScore")
                ->orderBy("pko_main_history.sportScore", "DESC")
                ->orderBy("pko_sport_score.sportScore", "DESC")
                ->get();
            return $getNoots;
